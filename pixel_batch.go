...
type SpriteBatchSystem struct {
	// Group sprites by source image for batching
	batches map[*ebiten.Image][]SpriteBatchEntry

	// Pre-allocated slices to avoid allocations
	entryPool []SpriteBatchEntry
...
// Global sprite batch system
var (
	spriteBatchSys     *SpriteBatchSystem
	spriteBatchSysOnce sync.Once
)

// GetSpriteBatchSystem returns the singleton sprite batch system
func GetSpriteBatchSystem() *SpriteBatchSystem {
	spriteBatchSysOnce.Do(func() {
		spriteBatchSys = &SpriteBatchSystem{
			batches:   make(map[*ebiten.Image][]SpriteBatchEntry, 32),
			entryPool: make([]SpriteBatchEntry, 0, 256),
		}
	})
	return spriteBatchSys
}

// QueueSprite adds a sprite to the batch queue
func (sbs *SpriteBatchSystem) QueueSprite(entry SpriteBatchEntry) {
	sbs.batches[entry.SrcImage] = append(sbs.batches[entry.SrcImage], entry)
	sbs.spritesQueued++
}

// Flush draws all batched sprites and clears the queue
func (sbs *SpriteBatchSystem) Flush(screen *ebiten.Image, shader *ebiten.Shader) {
	if screen == nil {
		return
	}

	for srcImage, entries := range sbs.batches {
		for _, entry := range entries {
			...
			// Apply scaling
			if entry.ScaleX != 1.0 || entry.ScaleY != 1.0 {
				opts.GeoM.Scale(entry.ScaleX, entry.ScaleY)
			}

			// Apply flipping
			bounds := srcImage.Bounds()
			destWidth := float64(bounds.Dx()) * entry.ScaleX
			destHeight := float64(bounds.Dy()) * entry.ScaleY
			...
			// Apply position
			opts.GeoM.Translate(entry.DstX, entry.DstY)
			opts.Filter = ebiten.FilterNearest

			// Draw with shader if available and requested
			if entry.UseShader && shader != nil {
				var shaderOpts ebiten.DrawRectShaderOptions
				shaderOpts.GeoM = opts.GeoM
				shaderOpts.Images[0] = srcImage
				screen.DrawRectShader(bounds.Dx(), bounds.Dy(), shader, &shaderOpts)
			} else {
				screen.DrawImage(srcImage, &opts)
			}
		}
		sbs.batchesDrawn++
	}

	// Clear batches (reuse map)
	for k := range sbs.batches {
		sbs.batches[k] = sbs.batches[k][:0] // Clear slice, keep capacity
	}
	sbs.spritesQueued = 0
}

// Reset clears the batch system
func (sbs *SpriteBatchSystem) Reset() {
	for k := range sbs.batches {
		delete(sbs.batches, k)
	}
	sbs.spritesQueued = 0
	sbs.batchesDrawn = 0
}
...
// Stats returns batch system statistics
func (sbs *SpriteBatchSystem) Stats() (queued int64, batches int64) {
